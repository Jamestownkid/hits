...
export async function generateEditManifest(
  ...
  // get mode config
  const modeConfig = VIDEO_MODES[mode] || VIDEO_MODES.custom

  // get the edit catalog from brain - this is what tells claude what effects exist
  const editCatalog = brain.generateEditCatalog()
  
  // get edits available for this mode
  const availableEdits = brain.getForMode(mode)
  const editIds = availableEdits.map(e => e.meta.id)

  // format transcript for claude
  const formattedTranscript = transcript.segments.map(seg => 
